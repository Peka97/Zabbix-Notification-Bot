...
class ZabbixAPI:
    ...
    def get_graph(self, settings: dict) -> tuple[bytes, str, int]:
        ...
        with requests.Session() as session:
            ...
            with session.get(
                ...
                raise requests.exceptions.ConnectionError(
                    f"Graph for \"item ID {settings['itemid']}\" not found.")
    ...
    def get_availability_report(self):
        url = self._url + \
            f"/report2.php?mode=1&from={config.availability_report_pediod}&to=now&filter_groupid=0&filter_templateid=10564&tpl_triggerid=23176&hostgroupid=0&filter_set=1"
